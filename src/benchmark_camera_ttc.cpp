...
int main(int argc, const char *argv[])
{
    ...
    // benchmark test
    vector<string> detectorTypes   = {"FAST", "BRISK", "ORB", "AKAZE", "SIFT"};
    vector<string> descriptorTypes = {"BRIEF", "ORB", "FREAK", "AKAZE", "SIFT", "BRISK"};

    fstream fout;
    fout.open("../ttc_camera.csv", ios::out | ios::app); 
    fout << "detector,descriptor,image_id, ttc" << endl;
    ...
    {   for (auto det : detectorTypes) 
        {   
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
            {
                ...
                cv::Mat descriptors;
                string descriptorType = des; // BRISK, BRIEF, ORB, FREAK, AKAZE, SIFT
                descKeypoints((dataBuffer.end() - 1)->keypoints, (dataBuffer.end() - 1)->cameraImg, descriptors, descriptorType);

                // push descriptors for current frame to end of data buffer
                (dataBuffer.end() - 1)->descriptors = descriptors;

                // CLUSTER AND STORE KEYPOINTS TO BBOXES FOR EACH DATAFRAME
                map<int, int> kpt2box;
                mapKeypoint2ROI((dataBuffer.end() - 1)->keypoints, (dataBuffer.end() - 1)->boundingBoxes, kpt2box);
                (dataBuffer.end() - 1)->keypoint2Box = kpt2box;
            ...
            {
                cout << i << ", " << fixed << setprecision(6) << ttc[i] << endl;
            }
            // write to csv file
            for (int i=0; i<ttc.size(); ++i)
            {
                fout << det << "," << des << "," << i << "," << fixed << setprecision(6) << ttc[i] << endl;
            }
            dataBuffer.clear();
        }       
    }
